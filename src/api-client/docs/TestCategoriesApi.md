...
| [**syncTestsCreate**](TestCategoriesApi.md#synctestscreate)         | **POST** /testCategories/{id}/sync-tests/ |             |
...
> TestCategory syncTestsCreate(id, testCategory)
...
import type { SyncTestsCreateRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // TestCategory
    testCategory: ...,
  } satisfies SyncTestsCreateRequest;

  try {
    const data = await api.syncTestsCreate(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }

...
const ActivityFeed: React.FC = () => {
    ...
    const expandedRowRender = (record: EventLogType) => {
        ...
        return (
            <div style={{ padding: '10px', background: '#fafafa', borderRadius: '4px', wordWrap: 'break-word', overflowWrap: 'anywhere' }}>
                <p><strong>Full Description:</strong></p>
                <div style={{ whiteSpace: 'pre-wrap', marginBottom: '10px', wordWrap: 'break-word', overflowWrap: 'anywhere' }}>{record.description}</div>

                {record.user && <p><strong>User:</strong> {record.user}</p>}
                {record.courseID && <p><strong>Course ID:</strong> {record.courseID}</p>}

                {prettyMeta && prettyMeta !== "{}" && prettyMeta !== "" && (
                    <>
                        <p><strong>Meta Data:</strong></p>
                        <pre style={{ fontSize: '12px', background: '#eee', padding: '8px', borderRadius: '4px', whiteSpace: 'pre-wrap', wordWrap: 'break-word', overflowWrap: 'anywhere' }}>
                            {prettyMeta}
                        </pre>
                    </>
                )}
    ...
    return (
        <div style={{ maxWidth: '100%', overflow: 'hidden' }}>
            <Card
                ...
                bodyStyle={{ padding: 0 }}
                extra={
                    <Button icon={<ReloadOutlined />} onClick={() => fetchLogs(1, pageSize)}>Refresh</Button>
                }
            >
                <Table
                    dataSource={logs}

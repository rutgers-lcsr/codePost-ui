...
export interface IRubricManagerHelpers {
  loadAssignmentRubric: (assignment: AssignmentType, shouldLoadInstances: boolean, shouldLoadFeedback: boolean) => Promise<void>;
  ...
  saveRubric: (categories: RubricCategoryType[], comments: IRubricCategoryToRubricCommentsMap, unsavedComments: RubricCommentType[], deletedComments: RubricCommentType[], unsavedCategories: RubricCategoryType[], deletedCategories: RubricCategoryType[], resolved: Record<number, RESOLUTION>, demoMode?: boolean) => Promise<{ rubricCategories: RubricCategoryType[]; rubricComments: IRubricCategoryToRubricCommentsMap }>;
  ...
  buildLinkedList: (deletedComments: RubricCommentType[], editedComments: RubricCommentType[], resolved: Record<number, RESOLUTION>, instanceLists: Record<number, number[]>) => { edited: RubricCommentType[]; deleted: RubricCommentType[] };
  onSave: (fnc?: (rubric: unknown) => void, demoMode?: boolean) => Promise<void>;
  buildCommentMap: (categories: RubricCategoryType[], comments: RubricCommentType[]) => IRubricCategoryToRubricCommentsMap;
  ...
  onLinkedCommentsResolve: (comment: RubricCommentType, resolution: RESOLUTION, fnc?: () => void) => void;
  onLinkedConfirmCancel: () => void;
  onLinkedConfirmAccept: (fnc?: () => void) => void;
  onBack: () => void;
  ...
  onCommentDragEnd: (result: { destination?: { droppableId: string; index: number } | null; source: { index: number } }) => void;
}
...
export interface IRubricManagerProps {
  ...
  setRubric?: (rubric: { rubricCategories: RubricCategoryType[]; rubricComments: IRubricCategoryToRubricCommentsMap }) => void;
...
const RubricManager: React.FC<IRubricManagerProps> = (props) => {
  ...
  const buildCommentMap = useCallback((categories: RubricCategoryType[], comments: RubricCommentType[]): IRubricCategoryToRubricCommentsMap => {
  ...
  const loadFeedbackScores = useCallback(async (rubricComments: RubricCommentType[]) => {
  ...
  // Load instance lists
  const loadInstanceLists = useCallback(async (rubricComments: RubricCommentType[]): Promise<Record<number, number[]>> => {
  ...
  const loadAssignmentRubric = useCallback(async (
    assignment: AssignmentType,
  ...
  const deleteLinkedComments = useCallback(async (rubricComment: RubricCommentType) => {
    let comments: number[] = [];
  ...
  const unlinkLinkedComments = useCallback(async (rubricComment: RubricCommentType) => {
    let comments: number[] = [];
    if (store.instanceLists[rubricComment.id]) {
      comments = store.instanceLists[rubricComment.id];
    } else {
  ...
  const saveRubric = useCallback(async (
    ...
    const promises = demoMode ? [] : categories.map(category => {
      if (category.id < 0) {
    ...
    // Delete comments
    const deleteCommentPromises = demoMode ? [] : deletedComments.map(rubricComment => {
  ...
  const onSave = useCallback(async (fnc?: (rubric: unknown) => void, demoMode?: boolean) => {
    ...
    const conflicts = buildLinkedList(stateAfterLoad.deletedComments, stateAfterLoad.unsavedComments, stateAfterLoad.resolutions, newLists);
  ...
  // Move category
  const moveCategory = useCallback((category: RubricCategoryType, direction: DIRECTION) => {
  ...
  // Add rubric category
  const addRubricCategory = useCallback((name?: string) => {
    const payload = {
  ...
  // Add rubric comment
  const addRubricComment = useCallback((category: RubricCategoryType) => {
    const payload = {
      id: -1, // Will be replaced by store
      text: '',
      pointDelta: 0,
  ...
  // Set new rubric
  const setNewRubric = useCallback((categories: RubricCategoryType[], comments: IRubricCategoryToRubricCommentsMap) => {
    store.initialize(categories, comments);
    onSave(undefined);
  }, [onSave, store]);

  // Replace rubric
  const replaceRubric = useCallback((categories: RubricCategoryType[], comments: IRubricCategoryToRubricCommentsMap) => {
  ...
  const onLinkedCommentsResolve = useCallback((comment: RubricCommentType, resolution: RESOLUTION, fnc?: () => void) => {
  ...
  const onLinkedConfirmAccept = useCallback((fnc?: () => void) => {
  ...
  // Comment drag end
  const onCommentDragEnd = useCallback((result: {
  ...
  }, [loadAssignmentRubric, props.assignment, props.reloadInterval, props.shouldLoadFeedback, props.shouldLoadInstanceLists]);

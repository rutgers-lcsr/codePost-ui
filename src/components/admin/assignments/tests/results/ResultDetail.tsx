...
export const ResultDetail = (props: IProps) => {
  ...
  const isInactive = (submission: SubmissionType) => {
    ...
    if (filterCase) {
      return (
        SubmissionTest.getLatest(props.testsBySubmission[submission.id]).find(
    ...
    return (
      SubmissionTest.getLatest(props.testsBySubmission[submission.id]).find((t) => sameStatus(t, filterStatus)) ===
      undefined
    );
  };
  ...
  const categoriesToShow = filterCategory ? [filterCategory] : props.categories;
  ...
  return (
    <Modal width={1000} visible={props.visible} footer={[closeButton]} onCancel={props.onCancel}>
      ...
      <Layout>
        ...
        <Layout.Content>
          {filterCase && testsToShow.length > 0 ? (
            ...
            <TestsList
              tests={testsToShow}
              cases={props.casesByCategory}
              categories={categoriesToShow}
              hideNotRun={filterStatus !== undefined}
              hideSummary={(filterCategory || filterCase || filterStatus) !== undefined}
            />
          ) : (

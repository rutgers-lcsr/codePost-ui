          ...
          <TableBody>
            {Object.keys(submissionsByStudent).map(studentID => {
              return (
                <TableRow
                  key={studentID}
                  onClick={changeActiveStudent.bind(this.props, studentID)}
                >
                  <TableColumn>
                    {submissionsByStudent[studentID].profile.username}
                  </TableColumn>
                  {this.props.assignments.map(assignment => {
                    const submission =
                      submissionsByStudent[studentID].submissionsByAssignment[
                        assignment.id
                      ];
          ...
          <DataTable plain={true}>
            ...
            <TableBody>
              {assignments.map(assignment => {
                const submission =
                  submissionsByStudent[activeStudent].submissionsByAssignment[
                    assignment.id
                  ];
